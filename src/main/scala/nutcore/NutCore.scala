...
trait HasNutCoreParameter {
  ...
  val Issue_Num = 2
  val Queue_num = 16
  val Polaris_Independent_Bru = 1 //0 or 1
  val Polaris_SIMDU_WAY_NUM = 2   //1 or 2
}
...
class NutCore(implicit val p: NutCoreConfig) extends NutCoreModule {
  ...
  if (EnableOutOfOrderExec) {
    ...
    */
    ...
    // dtlb
    val dtlb = EmbeddedTLB(in = backend.io.dmem, mem = dmemXbar.io.in(2), flush = backend.io.flush(0), csrMMU = backend.io.memMMU.dmem, enable = HasDTLB)(TLBConfig(name = "dtlb", totalEntry = 64))
