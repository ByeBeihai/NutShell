...
class NutCore(implicit val p: NutCoreConfig) extends NutCoreModule {
  ...
  if (EnableOutOfOrderExec) {
    ...
    */
    ...
    // dtlb
    val dtlb = EmbeddedTLB(in = backend.io.dmem, mem = dmemXbar.io.in(2), flush = RegNext(backend.io.flush(0)), csrMMU = backend.io.memMMU.dmem, enable = HasDTLB)(TLBConfig(name = "dtlb", totalEntry = 64))
