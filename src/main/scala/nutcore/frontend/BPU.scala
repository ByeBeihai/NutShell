...
class BPU_signle extends NutCoreModule { //bpu for single inst
  val io = IO(new Bundle {
    val in = new Bundle { val pc = Flipped(Valid((UInt(VAddrBits.W)))) }
    val out = new RedirectIO
    val flush = Input(Bool())
    val useGshare = Input(Bool())
  })
  ...
  val GHR = Reg(UInt(log2Up(NRbtb).W)) //Global History Register for Gshare 


  val pht = Mem(NRbtb, UInt(2.W))
  val phtTaken = RegEnable(Mux (io.useGshare,pht.read(btbAddr.getIdx(io.in.pc.bits)^GHR)(1),pht.read(btbAddr.getIdx(io.in.pc.bits))(1)), io.in.pc.valid)
  ...
  when (reqLatch.valid && ALUOpType.isBranch(reqLatch.fuOpType)) {
    ...
    val newCnt = Mux(taken, cnt + 1.U, cnt - 1.U)
    val wen = (taken && (cnt =/= "b11".U)) || (!taken && (cnt =/= "b00".U))
    GHR := Cat(taken,GHR (log2Up(NRbtb)-1,1))
