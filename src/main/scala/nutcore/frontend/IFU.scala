...
class IFU_single extends NutCoreModule with HasResetVector { 
  //ifu for single inst 
  val io = IO(new Bundle {
    val imem = new SimpleBusUC(userBits = VAddrBits*2 + 4, addrBits = VAddrBits)
    val out = Decoupled(new CtrlFlowIO)
    val redirect = Flipped(new RedirectIO) //from backend
    val flushVec = Output(UInt(4.W))
    val bpFlush = Output(Bool())
    val ipf = Input(Bool())
  })
  ...
  val bp_sU = Module(new BPU_signle)
  val pnpc = bp_sU.io.out.target
  val npc = Mux(io.redirect.valid, io.redirect.target, Mux(bp_sU.io.out.valid, pnpc, snpc))

  bp_sU.io.in.pc.valid := io.imem.req.fire() // only predict when Icache accepts a request
  bp_sU.io.in.pc.bits := npc  // predict one cycle early

  bp_sU.io.flush := io.redirect.valid
  bp_sU.io.useGshare := true.B //define useGshare algorithm for bp
  ...
  io.imem.req.bits.apply(addr = Cat(pc(VAddrBits-1,1),0.U(1.W)), //cache will treat it as Cat(pc(63,3),0.U(3.W))
    size = "b11".U, cmd = SimpleBusCmd.read, wdata = 0.U, wmask = 0.U, user = Cat(npc(VAddrBits-1, 0), pc(VAddrBits-1, 0)))   
  io.imem.req.valid := io.out.ready
  io.imem.resp.ready := io.out.ready || io.flushVec(0)
