...
class ILABundle extends NutCoreBundle {
  ...
  val IsuValid = UInt(2.W)
  val IsuInValid = UInt(2.W)
  val IsuReady = UInt(2.W)
  val ExuValid = UInt(FuType.num.W)
  val ExuInValid = UInt(FuType.num.W)
  val ExuReady = UInt(FuType.num.W)
  val WbuInValid = UInt(FuType.num.W)
  val lsureqvalid = UInt(1.W)
  val lsureqready = UInt(1.W)
  val lsurespvalid = UInt(1.W)
  val lsurespready = UInt(1.W)
  val lsupc = UInt(VAddrBits.W)
  val lsustate = UInt(3.W)
  val IsuPc0 = UInt(VAddrBits.W)
  val IsuPc1 = UInt(VAddrBits.W)
  val lsureqAddr = UInt(VAddrBits.W)
  val WbuRfwenVec = UInt(FuType.num.W)
  val vmEnable = UInt(1.W)
  val WbuInValidNext = UInt(FuType.num.W)
}
...
class NutShell(implicit val p: NutCoreConfig) extends Module with HasSoCParameter {
  ...
  if (p.FPGAPlatform) {
    ...
    BoringUtilsConnect(ila.IsuValid   ,"IsuValid")
    BoringUtilsConnect(ila.IsuInValid   ,"IsuInValid")
    BoringUtilsConnect(ila.IsuReady   ,"IsuReady")
    BoringUtilsConnect(ila.IsuPc0   ,"IsuPc0")
    BoringUtilsConnect(ila.IsuPc1   ,"IsuPc1")
    BoringUtilsConnect(ila.ExuValid   ,"ExuValid")
    BoringUtilsConnect(ila.ExuInValid   ,"ExuInValid")
    BoringUtilsConnect(ila.ExuReady   ,"ExuReady")
    BoringUtilsConnect(ila.WbuInValid   ,"WbuInValid")
    BoringUtilsConnect(ila.WbuRfwenVec,"WbuRfwen")
    BoringUtilsConnect(ila.lsureqvalid   ,"lsureqvalid")
    BoringUtilsConnect(ila.lsureqready   ,"lsureqready")
    BoringUtilsConnect(ila.lsurespvalid   ,"lsurespvalid")
    BoringUtilsConnect(ila.lsurespready   ,"lsurespready")
    BoringUtilsConnect(ila.lsupc   ,"lsupc")
    BoringUtilsConnect(ila.lsustate   ,"lsustate")
    BoringUtilsConnect(ila.lsureqAddr   ,"lsureqAddr")
    BoringUtilsConnect(ila.vmEnable,"DTLBENABLE")
    BoringUtilsConnect(ila.WbuInValidNext,"WbuInValidNext")
  }
}
