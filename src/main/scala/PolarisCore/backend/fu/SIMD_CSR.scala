...
trait SIMD_HasCSRConst {
  ...
  // Currently, PolarisCore uses perfcnt csr set instead of standard Machine Counter/Timers 
...
class SIMD_CSR(implicit val p: PolarisCoreConfig) extends PolarisCoreModule with SIMD_HasCSRConst{
  ...
  val tvalWen = !(hasInstrPageFault || hasLoadPageFault || hasStorePageFault || hasLoadAddrMisaligned || hasStoreAddrMisaligned) || raiseIntr // in PolarisCore-riscv64, no exception will come together with PF
...
class new_SIMD_CSR(implicit val p: PolarisCoreConfig) extends PolarisCoreModule with HasCSRConst{
  ...
  //connect exu
  io.in.ready := true.B
  io.out.valid := valid && !io.ctrlIn.isPolarisCoreTrap
  io.out.bits := rdata
