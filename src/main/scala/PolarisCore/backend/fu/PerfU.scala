...
class PerfU(implicit val p: PolarisCoreConfig) extends PolarisCoreModule {
  ...
  val PolarisCoretrap = WireInit(false.B)
  BoringUtils.addSink(PolarisCoretrap, "PolarisCoretrap")
  ...
  if (!p.FPGAPlatform) {
    ...
    if (hasPerfCnt) {
      // display all perfcnt when PolarisCoretrap is executed
      val PrintPerfCntToCSV = true
      when (PolarisCoretrap) {
        printf("======== PerfCnt =========\n")
