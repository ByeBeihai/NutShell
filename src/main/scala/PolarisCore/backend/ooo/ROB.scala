...
class ROB(implicit val p: PolarisCoreConfig) extends PolarisCoreModule with HasInstrType with HasBackendConst with HasRegFileParameter{
  ...
  // In PolarisCore-Argo's backend, non-l/s int/exc will be sent dircetly to CSR when dispatch,
