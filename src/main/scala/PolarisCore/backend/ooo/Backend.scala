...
class Backend_ooo(implicit val p: PolarisCoreConfig) extends PolarisCoreModule with HasRegFileParameter with HasBackendConst{
  ...
  if (!p.FPGAPlatform) {
    ...
    val PolarisCoretrap = csrrs.io.out.bits.decode.ctrl.isPolarisCoreTrap && csrrs.io.out.valid
...
class new_Backend_inorder(implicit val p: PolarisCoreConfig) extends PolarisCoreModule with HasRegFileParameter{
  val io = IO(new Bundle {
    val in = Vec(2, Flipped(Decoupled(new DecodeIO)))
