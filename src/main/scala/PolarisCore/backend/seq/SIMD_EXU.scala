...
class new_SIMD_EXU(implicit val p: PolarisCoreConfig) extends PolarisCoreModule with HasInstrType {
...
//  io.out(fconvidx).bits.decode.ctrl.fReg.wen := fconv.io.decode.ctrl.fReg.wen
  ...
  */
  ...
  if (!p.FPGAPlatform) {
    ...
    val PolarisCoretrap = VecInit((0 to FuType.num-1).map(i => io.in(i).bits.ctrl.isPolarisCoreTrap && io.in(i).valid)).reduce(_||_)

    val tarpNo = PriorityMux(io.in.map(i => i.bits.ctrl.isPolarisCoreTrap).zipWithIndex.map{case(a,b)=>(a,b.U)})

    BoringUtils.addSink(cycleCnt, "simCycleCnt")
    BoringUtils.addSink(instrCnt, "simInstrCnt")
    BoringUtils.addSource(PolarisCoretrap, "PolarisCoretrap")
