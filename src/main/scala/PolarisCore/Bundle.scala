...
class CtrlSignalIO extends PolarisCoreBundle {
  ...
  val isPolarisCoreTrap = Output(Bool())
...
class DataSrcIO extends PolarisCoreBundle with HasLSUConst{
...
class new_SIMD_WriteBackIO() extends PolarisCoreBundle with HasPolarisCoreParameter with HasLSUConst with HasRegFileParameter{
...
class SIMD_CommitIO extends PolarisCoreBundle with HasLSUConst{
  val decode = new DecodeIO
  val isMMIO = Output(Bool())
  val intrNO = Output(UInt(XLEN.W))
