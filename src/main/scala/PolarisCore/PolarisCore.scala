...
trait HasPolarisCoreParameter extends HasLSUConst{
  ...
  val int_Forward_num = 4 /*+ Polaris_SIMDU_WAY_NUM + Polaris_SNN_WAY_NUM + (if(Polaris_Vector_LDST)(vector_rdata_width/XLEN -1 + vector_rdata_width/XLEN -1)else(0)) + 4*/
  val float_Forward_num = 1 /*+ Polaris_SIMDU_WAY_NUM + Polaris_SNN_WAY_NUM + (if(Polaris_Vector_LDST)(vector_rdata_width/XLEN -1 + vector_rdata_width/XLEN -1)else(0)) + 4*/
  val Commit_num = 2
...
trait HasPolarisCoreLog { this: RawModule =>
  implicit val moduleName: String = this.name
}

abstract class PolarisCoreModule extends Module with HasPolarisCoreParameter with HasPolarisCoreConst with HasExceptionNO with HasBackendConst with HasPolarisCoreLog with HasLSUConst
...
class PolarisCore(implicit val p: PolarisCoreConfig) extends PolarisCoreModule {
  class PolarisCoreIO extends Bundle {
    val imem = new SimpleBusC
    val dmem = new SimpleBusC
    val mmio = new SimpleBusUC
    val frontend = Flipped(new SimpleBusUC())
  }
  val io = IO(new PolarisCoreIO)
