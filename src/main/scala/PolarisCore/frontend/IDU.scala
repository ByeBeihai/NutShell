...
class Decoder(implicit val p: PolarisCoreConfig) extends PolarisCoreModule with HasInstrType {
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new CtrlFlowIO))
    val out = Decoupled(new DecodeIO)
    val isWFI = Output(Bool()) // require PolarisCoreSim to advance mtime when wfi to reduce the idle time in Linux
  ...
  io.out.bits.ctrl.isPolarisCoreTrap := (instr(31,0) === PolarisCoreTrap.TRAP) && io.in.valid
  ...
  io.out.bits.ctrl.isPolarisCoreTrap := (instr === PolarisCoreTrap.TRAP) && io.in.valid
...
class IDU(implicit val p: PolarisCoreConfig) extends PolarisCoreModule with HasInstrType {
  val io = IO(new Bundle {
